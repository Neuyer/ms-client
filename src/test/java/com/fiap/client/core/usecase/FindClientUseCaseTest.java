...
class FindClientUseCaseTest {
    ...
    void setUp() {
        ...
        findClientUseCase = new FindClientUseCase(clientGateway);
    }

    @Test
    void testExecuteSuccessfullyFindsClient() {
        ...
        Client foundClient = findClientUseCase.execute(document);

        assertNotNull(foundClient);
        assertEquals("John Doe", foundClient.getName());
        assertEquals(LocalDate.of(1990, 5, 20), foundClient.getBirthDate());
        assertEquals(document, foundClient.getDocument().getValue());

        verify(clientGateway, times(1)).findByDocument(document);
    }
    ...
    void testExecuteThrowsExceptionWhenClientNotFound() {
        ...
        Exception exception = assertThrows(IllegalStateException.class, () -> findClientUseCase.execute(document));
